 ...
 * @since 2022.04.20
 */
abstract class RijselComposeFragment<AggregateClass : RijselComposeFragmentAggregate, ViewModelClass : RijselComposeViewModel>
  : Fragment(),
    Rijselable<AggregateClass>, RijselComposeFragmentConfigurable
{

  private var rijselizer: Rijselizer<AggregateClass, RijselComposeFragment<AggregateClass, ViewModelClass>>? = null
  ...
  {
    rijselizer?.onCreate({
      super@RijselComposeFragment.onCreate(savedInstanceState)
    }, savedInstanceState)
  }
  ...
  {
    val viewModelOwner: ViewModelStoreOwner = if (viewModelContext() == RijselComposeFragmentConfigurable.ViewModelContext.Fragment) this@RijselComposeFragment else requireActivity()
  ...
  protected open fun observeStates()
  {
    viewLifecycleOwner.lifecycleScope.launch {
      viewModel?.stateManager?.state?.flowWithLifecycle(viewLifecycleOwner.lifecycle, Lifecycle.State.STARTED)?.collect {
        Timber.d("The state ${it.javaClass.simpleName} has been observed into the fragment ${this@RijselComposeFragment.javaClass.simpleName}")
