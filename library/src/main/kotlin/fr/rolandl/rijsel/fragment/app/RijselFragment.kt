 ...
 * @since 2018.11.07
 */
abstract class RijselFragment<AggregateClass : RijselFragmentAggregate, BindingClass : ViewDataBinding, ViewModelClass : RijselViewModel>
  : Fragment(),
    Rijselable<AggregateClass>, RijselFragmentConfigurable
{

  private var rijselizer: Rijselizer<AggregateClass, RijselFragment<AggregateClass, BindingClass, ViewModelClass>>? = null
  ...
  {
    rijselizer?.onCreate({
      super@RijselFragment.onCreate(savedInstanceState)
    }, savedInstanceState)
  }
  ...
  {
    val viewModelOwner: ViewModelStoreOwner = if (viewModelContext() == RijselFragmentConfigurable.ViewModelContext.Fragment) this@RijselFragment else requireActivity()
    ...
    getViewModelClass()?.let {
      ...
      arguments?.putSerializable(RijselViewModel.RIJSEL_VIEWMODEL_BEHAVIOR_EXTRA, viewModelBehavior())
  ...
  protected open fun observeStates()
  {
    viewModel?.stateManager?.state?.observe(viewLifecycleOwner) {
      Timber.d("The state ${it.javaClass.simpleName} has been observed into the fragment ${this@RijselFragment.javaClass.simpleName}")
